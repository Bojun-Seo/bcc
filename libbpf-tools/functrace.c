...
int main(int argc, char **argv)
{
	...
	*env.symbolname = '\0';
	...
	prev_key.start_nsec = -1;
	prev_key.pid_tid = -1;
	while(!bpf_map_get_next_key(fd, &prev_key, &curr_key)) {
		if (errno == ENOENT)
			break;

		err = bpf_map_lookup_elem(fd, &curr_key, &info);
		if (err < 0)
			goto cont;

		pid = curr_key.pid_tid >> 32;
		tid = curr_key.pid_tid;

		if (env.duration) {
			fprintf(os, "%llu\t%llu", curr_key.start_nsec,
				info.end_nsec - curr_key.start_nsec);
		} else {
			epoch_ns = boot_offset + curr_key.start_nsec;
